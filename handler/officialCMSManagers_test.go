...
func Test_officialCMSManagersHandler_get(t *testing.T) {
	...
	for _, tc := range tt {
		...
		router.Handle(path, NewOfficialCMSManagersHandler(NewBaseHandler(dbx.DB, dbx.Logger, uniqueIDProvider))).Methods("GET")
...
func Test_officialCMSManagersHandler_post(t *testing.T) {
	...
	for _, tc := range tt {
		...
		// Need to create a router that we can pass the request through so that the vars will be added to the context
		router := mux.NewRouter()
		router.Handle(path, NewOfficialCMSManagersHandler(NewBaseHandler(dbx.DB, dbx.Logger, uniqueIDProvider))).Methods("POST")
